...
**Supersedes:** the "MalaysiaHub" product framing (map-first Atlas). The
...
portals. It replaces MalaysiaHub at **malaysiahub.swymble.com**.
...
MalaysiaHub is a **production** Next.js 16 + React 19 app, not a prototype:
...
## 2. Migration: MalaysiaHub → MalaysiaHub
...
1. **Rebrand identity** — `site.ts`, wordmark, metadata/OG, README, `env`
   ...
   DNS (mirrors the malaysiahub worker; retire the old route or 301 it).
...
`../malaysiahub` repo (renamed) and syncs into swymble via
