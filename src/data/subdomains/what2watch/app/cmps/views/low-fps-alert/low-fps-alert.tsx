...
export const LowFpsAlert = () => {
  ...
  // Only worth interrupting the user when there is actually something to
  // lower: a heavier preset than Minimal/Mobile, or more than the minimum
  // 1,000 films. On the defaults the alert never shows.
  const canLowerQuality = preset
    ? [VOROFORCE_PRESET.contours, VOROFORCE_PRESET.depth].includes(preset) ||
      ...
      (cellLimit && cellLimit > CELL_LIMIT.xxxs)
    : false
  const warnLimit = 2
  ...
  useEffect(() => {
    if (performanceMonitor && canLowerQuality && openedCount < warnLimit) {
  ...
  }, [performanceMonitor, canLowerQuality, openedCount, open])
      ...
      <div className='flex flex-col p-4 md:p-6 lg:p-9 max-md:landscape:h-full max-md:landscape:justify-between'>
        <div className='flex flex-col gap-2 pt-4'>
          ...
          <p className='inline-flex pb-2 text-base text-zinc-600 max-md:pb-2 dark:text-zinc-300'>
            ...
            <span className='max-md:hidden'>Lower the settings?</span>
          </p>
        </div>
