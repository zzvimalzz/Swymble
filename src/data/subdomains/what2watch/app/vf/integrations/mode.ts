...
// The intro zoom animation must not play while the landing overlay still
// covers the screen (the user hasn't clicked Enter yet) - otherwise it would
// finish invisibly behind it, which also breaks "Replay Intro" in settings.
const whenEnteredApp = (callback: () => void) => {
  if (store.getState().enteredApp) {
    callback()
    return
  }
  const unsubscribe = store.subscribe(
    (state) => state.enteredApp,
    (enteredApp) => {
  ...
  if (initialMode === VOROFORCE_MODE.intro) {
    if (config.media.enabled && loader.loadingMediaLayers !== 0) {
      loader.listenOnce('idle', () => {
        ...
        ticker.listenOnce('tick', () => whenEnteredApp(handleIntro))
      })
    } else {
      whenEnteredApp(handleIntro)
    }
  } else {
