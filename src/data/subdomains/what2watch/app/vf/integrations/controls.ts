...
export const handleControls = () => {
  ...
  controls.listen('focused', (async ({ cell }: { cell: VoroforceCell }) => {
    // While a tile is selected, hovering other tiles must not swap the open
    // card - it only changes on an explicit new selection (click/space).
    if (store.getState().mode === VOROFORCE_MODE.select) return
    if (cell) setFilm(await getCellFilm(cell, store.getState().filters))
  }) as unknown as EventListener)

  controls.listen('selected', (async ({ cell }: { cell: VoroforceCell }) => {
    if (cell) {
      setFilm(await getCellFilm(cell, store.getState().filters))
      // controls.pinPointer()
    } else {
