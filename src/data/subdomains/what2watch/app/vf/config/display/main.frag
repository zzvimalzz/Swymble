...
// what2watch "gold cinema" grade: idle tiles fade into a warm amber duotone
// (deep espresso shadows -> gilded highlights) instead of upstream's neutral
// gray, so full color blooming in around the pointer reads as the signature
// interaction. Tuned to the site's gold accent (hsl(43 89% 58%)).
const vec4 GRAYSCALE_DUOTONE_DARK = vec4(0.075, 0.055, 0.028, 1);
const vec4 GRAYSCALE_DUOTONE_LIGHT = vec4(0.87, 0.72, 0.42, 1);
